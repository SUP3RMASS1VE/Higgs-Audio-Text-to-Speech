...
def initialize_engine(model_path, audio_tokenizer_path, device_arg=None) -> bool:
    ...
    try:
        ...
        if device == "cuda":
            logger.info("Attempting to fallback to CPU...")
            try:
                engine = HiggsAudioServeEngine(
                    model_name_or_path=model_path,
                    audio_tokenizer_name_or_path=audio_tokenizer_path,
                ...
                logger.info(f"Successfully initialized HiggsAudioServeEngine with model: {model_path} on device: cpu (fallback)")
                return True
            except Exception as cpu_e:
                logger.error(f"Failed to initialize engine on CPU as well: {cpu_e}")
                return False
        return False
...
def text_to_speech(
    ...
    global engine, DEVICE_ARG

    if engine is None:
        initialize_engine(DEFAULT_MODEL_PATH, DEFAULT_AUDIO_TOKENIZER_PATH, DEVICE_ARG)
